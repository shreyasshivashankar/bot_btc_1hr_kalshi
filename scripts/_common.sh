...
#
# Security model — zero-trust / BeyondCorp. The service is deployed with
# `--ingress=all --no-allow-unauthenticated` (see deploy/cloudrun.yaml), so:
#   1. Google Front End rejects requests without a Google-signed OIDC token
#      belonging to a principal with `roles/run.invoker` — 403 before the
#      request ever reaches the container.
#   2. The app layer requires `X-Admin-Token` matching the Secret Manager
#      value — defense-in-depth on top of IAM.
# Dropping to `ingress=internal-*` would only add value if we sat behind a
# VPC/GCLB; for a single-operator Cloud Run service it just breaks ops
# tooling without a meaningful security gain (the GFE+IAM layer is already
# mathematically sufficient against an unauthenticated attacker).
_admin_call() {
